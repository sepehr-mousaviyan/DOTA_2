...
public class GamePlay {
    private static GamePlay gameplay_instance = null;
    ...
    private final int CHUNK_SIZE; // size of unit moves
    private String teamName;
    private int turn_nth_client = 0;

    private GamePlay(String heroName1, String heroName2) {
    ...
    public static GamePlay getInstance(String heroName1, String heroName2)
    {
        if (gameplay_instance == null) {
            gameplay_instance = new GamePlay(heroName1, heroName2);
        }
        return gameplay_instance;
    }
    ...
    public static void abilityHit(int gunshot, Hero hero, Ability ability) {
        ...
        ArrayList<Unit> greenUnits = GreenUnits.getAll();
        ArrayList<Unit> redUnits = RedUnits.getAll();
        shots = getShots(gunshot, hero, ability, shots, greenUnits);
        shots = getShots(gunshot, hero, ability, shots, redUnits);
    }

    private static int getShots(int gunshot, Hero hero, Ability ability, int shots, ArrayList<Unit> greenUnits) {
    ...
    /**
     * destroy and delete unit
     * @param unit that should be destroy
     */
    ...
    /**
     * handle action message and do the action
     * @param clients list of all clients
     */
    private void receiveHeroData(ArrayList<Client> clients) {
        String data = sendMessageSetTurn(clients);
    ...
    /**
     * determine turn of which client to send action
     * @param clients list of all client
     * @return string of respond
     */
    private String sendMessageSetTurn(ArrayList<Client> clients){
        turn_nth_client++;
        if (turn_nth_client > 1) {
            turn_nth_client = 0;
        }
        Client client = clients.get(turn_nth_client);
        client.send(MessageMaker.message("ok", "startTurn").toString());
    ...
    /**
     * regenerate all mana of heroes
     */
    private void manaRegenerateAll() {
